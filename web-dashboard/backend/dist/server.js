...
app.post('/api/user/buy-key-with-money', authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    if (!db)
...
app.get('/api/titles', (req, res) => {
    ...
    try {
        const rows = db.prepare('SELECT username, title_text, color_code, is_bold FROM player_titles').all();
        const titlesMap = {};
        for (const r of rows) {
...
app.delete('/api/admin/titles/:username', authenticateToken, requireAdmin, async (req, res) => {
    const { username } = req.params;
    if (!db)
        return res.status(500).json({ success: false, message: '資料庫連線不可用' });
    try {
...
app.post('/api/admin/give-keys', authenticateToken, async (req, res) => {
    const { username, amount } = req.body;
    if (!username || typeof amount !== 'number' || amount <= 0) {
