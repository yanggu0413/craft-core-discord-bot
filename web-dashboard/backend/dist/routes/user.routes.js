...
router.get('/user/profile', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
...
router.post('/user/checkin', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
...
router.post('/user/upgrade', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
...
router.get('/user/fakeplayers', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
...
router.post('/user/fakeplayers/action', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
...
// GET /api/user/homes
router.get('/user/homes', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    try {
...
router.delete('/user/homes/:name', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
...
// GET /api/lockboxes
router.get('/lockboxes', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
    try {
        ...
        try {
            const possiblePaths = [
                path_1.default.resolve(__dirname, '../../../../config/craft-core-shop/lockboxes.json'),
                path_1.default.resolve(__dirname, '../../../../../fabric-mod/config/craft-core-shop/lockboxes.json'),
                path_1.default.resolve('config/craft-core-shop/lockboxes.json')
            ];
            for (const p of possiblePaths) {
                if (fs_1.default.existsSync(p)) {
                    const raw = fs_1.default.readFileSync(p, 'utf8');
...
router.post('/lockboxes/update', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
...
router.post('/tasks/claim', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
...
router.post('/playtime/exchange', wsClient_1.authenticateToken, async (req, res) => {
    const user = req.user;
    if (!user)
        return res.status(401).json({ success: false, message: '尚未登入' });
    const username = user.mc_username;
