...
app.post('/api/admin/announcements', async (req: CustomRequest, res: Response) => {
  ...
  if (db) {
    try {
      const stmt = db.prepare('INSERT INTO announcements (title, content, scope, impact, publisher) VALUES (?, ?, ?, ?, ?)');
  ...
  // 3. 發送至 Discord Webhook (若有設定環境變數)
...
app.get('/api/announcements', async (_req: Request, res: Response) => {
  if (!db) return res.json({ success: true, announcements: [] });
  try {
    const list = db.prepare('SELECT * FROM announcements ORDER BY id DESC LIMIT 20').all();
...
app.post('/api/warp-submissions', authenticateToken, async (req: CustomRequest, res: Response) => {
  const user = req.user;
  if (!user) return res.status(401).json({ success: false, message: '尚未登入' });
...
app.post('/api/admin/warp-submissions/:id/approve', async (req: CustomRequest, res: Response) => {
  const { id } = req.params;
  if (!db) return res.status(500).json({ success: false, message: '資料庫未連結' });

  try {
    const sub: any = db.prepare('SELECT * FROM warp_submissions WHERE id = ?').get(id);
    if (!sub) return res.status(404).json({ success: false, message: '找不到該審核紀錄' });
    ...
    for (const p of possiblePaths) {
      if (fs.existsSync(p)) { warpPath = p; break; }
    }
    ...
    if (fs.existsSync(warpPath)) {
      try { warpsMap = JSON.parse(fs.readFileSync(warpPath, 'utf8')) || {}; } catch (e) {}
    }
    ...
    fs.mkdirSync(path.dirname(warpPath), { recursive: true });
    fs.writeFileSync(warpPath, JSON.stringify(warpsMap, null, 2), 'utf8');
...
app.post('/api/admin/warp-submissions/:id/reject', async (req: CustomRequest, res: Response) => {
  const { id } = req.params;
  if (!db) return res.status(500).json({ success: false, message: '資料庫未連結' });
...
app.post('/api/admin/backup/trigger', authenticateToken, requireAdmin, async (req: CustomRequest, res: Response) => {
  try {
...
app.get('/api/admin/backup/status', authenticateToken, requireAdmin, async (req: CustomRequest, res: Response) => {
  try {
...
app.get('/api/admin/tickets', authenticateToken, requireAdmin, (req: any, res: any) => {
  if (!db) return res.status(500).json({ success: false, message: '資料庫未連結' });
...
app.get('/api/admin/tickets/:ticket_id', authenticateToken, requireAdmin, (req: any, res: any) => {
  if (!db) return res.status(500).json({ success: false, message: '資料庫未連結' });

  const ticketId = req.params.ticket_id;
  try {
    const row = db.prepare('SELECT * FROM tickets WHERE ticket_id = ? OR channel_id = ?').get(ticketId, ticketId) as any;
    if (!row) {
