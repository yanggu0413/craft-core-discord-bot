  ...
  return (
    <div className="space-y-6 text-left">
      <PageHeader
        ...
        kpis={[
          { label: "總發行幣", value: `$${(stats.totalCirculation || 0).toLocaleString()}`, icon: DollarSign },
          { label: "伺服器 TPS", value: `${(serverTps || 20).toFixed(1)}`, icon: Activity },
          { label: "箱子商店數", value: `${stats.totalShopsCount || 0} 間`, icon: TrendingUp },
          { label: "累計營業稅", value: `$${Math.floor(stats.accumulatedSalesTax || 0).toLocaleString()}`, icon: ShieldCheck }
        ]}
      />
          ...
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              <div className="flex items-center space-x-4">
                <div className="relative shrink-0">
                  <img 
                    ...
                    alt={username}
                    className="w-14 h-14 rounded-lg border border-border bg-muted shrink-0"
                  />
              ...
              <div className="flex items-center space-x-3 w-full lg:w-auto justify-between lg:justify-end border-t lg:border-t-0 border-border pt-4 lg:pt-0">
                <div className="text-left px-4 py-2 bg-muted/40 border border-border rounded-md min-w-28">
                  <p className="text-[10px] uppercase font-semibold tracking-wider text-muted-foreground">連續簽到</p>
                  <p className="text-sm font-bold font-mono text-foreground mt-0.5">{checkinStreak} 天</p>
                </div>
                <div className="text-left px-4 py-2 bg-muted/40 border border-border rounded-md min-w-28">
                  <p className="text-[10px] uppercase font-semibold tracking-wider text-muted-foreground">抽獎鑰匙</p>
                  <p className="text-sm font-bold font-mono text-foreground mt-0.5">{keysCount} 把</p>
                </div>
                {onNavigateToTab && (
                  <Button
        ...
        <Card className="border-border">
          <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <Badge variant="outline" className="text-[10px]">限時活動</Badge>
        ...
        <div className="lg:col-span-7 space-y-6">
          <Card>
            <CardHeader className="pb-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">即時圖表</Badge>
              </div>
            </CardHeader>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 border-b border-border">
              ...
              {onNavigateToTab && (
                <Button variant="ghost" size="sm" onClick={() => onNavigateToTab('tasks')} className="text-xs h-7">
                  查看全部
                </Button>
              )}
            </CardHeader>
              ...
              {dailyTasks && dailyTasks.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {dailyTasks.slice(0, 4).map((task, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-3 border border-border bg-muted/30 rounded-lg flex items-center justify-between">
          ...
          <Card>
            <CardHeader className="pb-3 border-b border-border">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline">即時排名</Badge>
              </div>
            </CardHeader>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-2.5">
                            <img 
                              ...
                              alt={player.username}
                              className="w-5 h-5 rounded-[3px] border border-border bg-muted"
                            />
