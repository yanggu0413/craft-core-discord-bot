...
public class DiscordCommand {
    ...
    public static void register() {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(CommandManager.literal("discord")
                    .executes(context -> {
                        context.getSource().sendMessage(Text.literal("§b[Craft-Core] §fDiscord 邀請連結：§a" + ConfigManager.getConfig().discordInvite));
                        return 1;
                    })
    ...
    private static int initiateBind(CommandContext<ServerCommandSource> context) {
        ...
        if (player == null) {
            source.sendMessage(Text.literal("此指令只能由遊戲內玩家執行。"));
            return 0;
        }

        CraftCoreWSClient client = CraftCoreMod.getWSClient();
        if (client == null || !client.isAuthenticated()) {
            source.sendMessage(Text.literal("§c[Craft-Core] §f機器人連線已中斷，請稍後再試！"));
            return 0;
        }
        ...
        source.sendMessage(Text.literal("§b[Craft-Core] §f正在向 Discord 申請綁定驗證碼..."));
        return 1;
    }
