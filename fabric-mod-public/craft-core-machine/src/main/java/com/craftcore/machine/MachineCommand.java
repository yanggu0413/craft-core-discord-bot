    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("machine")
                .executes(context -> {
                    ...
                    if (player != null) {
                        player.sendSystemMessage(Component.literal("§b[Machine] 輸入 /machine apply <名稱> 提交機器審核，或 /machine admin list 查看清單。"));
                    }
                    return 1;
                })
                ...
                .then(Commands.literal("admin")
                        ...
                        .then(Commands.literal("approve")
                                .then(Commands.argument("id", StringArgumentType.string())
                                        .suggests((context, builder) -> SharedSuggestionProvider.suggest(MachineManager.getPendingMachines().stream().map(m -> m.id), builder))
                                        .then(Commands.argument("tier", StringArgumentType.string())
                                                .suggests((context, builder) -> SharedSuggestionProvider.suggest(new String[]{"T1", "T2", "T3"}, builder))
                                                .executes(context -> {
