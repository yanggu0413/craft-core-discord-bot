...
echo [1/7] Stopping existing PM2 processes...
...
echo [4/7] Building Wiki Documentation (VitePress)...
cd docs
call npm run docs:build
cd ..
echo.

echo [5/7] Starting Discord Bot...
call pm2 start src/index.js --name craft-core-bot --cwd discord-bot
echo.

echo [6/7] Starting Web Dashboard Backend (Prod)...
call pm2 start dist/server.js --name craft-core-backend --cwd web-dashboard/backend
echo.

echo [7/7] Serving Web Dashboard Frontend (Prod)...
...
echo   * Wiki Docs can be served directly by Caddy
echo.
