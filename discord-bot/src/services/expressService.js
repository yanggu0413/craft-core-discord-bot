  ...
  try {
    ...
    for (const mail of pending) {
      try {
        ...
        const giveResult = await session.executeCommand(giveCmd, 'System');

        if (giveResult.success) {
          await OfflineMailRepository.markMailDelivered(mail.id);
          // Send tellraw notification
          const msgJson = JSON.stringify({
            text: `📦 [快遞] 收到來自 ${mail.sender_username} 的快遞：${mail.item_id.replace('minecraft:', '')} x${mail.quantity}！`,
            color: 'gold'
          });
          await session.executeCommand(`tellraw "${username}" ${msgJson}`, 'System');
          logger.info(`Successfully delivered mail #${mail.id} to ${username}`);
        } else {
          logger.error(`Failed to execute give command for mail #${mail.id}`, { output: giveResult.output });
        }
      } catch (err) {
