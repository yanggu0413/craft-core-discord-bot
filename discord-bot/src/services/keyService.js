...
async function handleLottery(interaction) {
  ...
  try {
    ...
    await session.executeCommand(`title "${binding.mc_username}" subtitle {"text":"獲得了 ${prize.name}","color":"gold"}`, 'System');
    await session.executeCommand(`playsound minecraft:entity.player.levelup master "${binding.mc_username}"`, 'System');
  } catch (err) {
    logger.error('Failed to execute lottery command in game', { error: err, username: binding.mc_username });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'Minecraft 帳號', value: `\`${binding.mc_username}\``, inline: true },
      { name: '獲得獎勵', value: `🎁 **${prize.name}**`, inline: true },
