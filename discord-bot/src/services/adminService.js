...
async function handleCoBrandSubmit(interaction) {
  ...
  try {
    if (isDiscord) {
      ...
      return interaction.editReply({
        content: `✅ 成功發送聯名獎勵 (6 把鑰匙) 給 Discord 用戶 <@${discordId}>！`
      });
    } else {
      await UserRepository.addKeysByMcUsername(resolvedTarget, 6);
      return interaction.editReply({
        content: `✅ 成功發送聯名獎勵 (6 把鑰匙) 給 Minecraft 玩家 \`${resolvedTarget}\`！`
      });
    }
  } catch (error) {
    return interaction.editReply({
