...
2. 貨幣與經濟體系：
   - 伺服器貨幣為「金幣 ($)」。獲得途徑：每日簽到 /checkin、幸運大抽獎 /luckydraw、親自至玩家箱子商店買賣物資。
   - 金幣轉帳：/pay <玩家> <金額>（帶有二次確認與防誤轉機制）；離線玩家可用 /express 發送離線包裹與附言金幣。
3. 商店與交易：
   - 箱子商店 (Chest Shop)：玩家可在領地內設置告示牌與箱子建立實體商店進行買賣。
   - 輸入 /shop 開啟商店管理面板（查看全服商店清單與導航）。
4. 福利簽到與抽獎：
...
7. 社群與假人系統：
   - Discord 帳號綁定 /discord link：生成 6 位數驗證碼，至 Discord 私訊機器人即可連動綁定身分組。
   - 假人控制台 /fp：可召喚掛機假人（用於掛生怪磚或農場），具備斷線自動重連。
...
1. ❌ 沒有 遙控買賣/遠端付手續費購買物品功能（買賣物品必須親自前往箱子商店現場交易！）。
2. ❌ 沒有 Web 網頁後台（所有網頁功能已 100% 遷移回 Minecraft 原生箱子 GUI！）。
3. ❌ 沒有 地皮插件 (PlotSquared)、沒有地皮世界、沒有公會/陣營 (Factions/Towny)。
4. ❌ 沒有 點數儲值網頁、沒有課金管道（完全靠遊戲內簽到與努力）。
5. ❌ 沒有 Paper 裝備模組（主伺服器為原生 Fabric 26.2）。
6. ❌ 沒有 多世界分區（只有 Overworld、Nether、End 三個標準原版維度）。`;
