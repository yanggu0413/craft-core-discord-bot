...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('封鎖')
    .setDescription('封鎖 Minecraft 玩家（管理員專用）')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(option =>
      option.setName('玩家名稱').setDescription('要封鎖的 Minecraft ID').setRequired(true)
    )
    .addStringOption(option =>
      option.setName('原因').setDescription('封鎖原因').setRequired(true)
    ),
    ...
    try {
      ...
      await interaction.editReply({
        content: result.success 
          ...
          : `封鎖玩家 \`${username}\` 失敗：\`${result.output}\``
      });
    } catch (error) {
