  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', async (message) => {
      try {
        ...
        if (!authenticated) {
          if (type === 'auth') {
            if (payload && payload.secret === config.websocket.secret) {
              if (payload.role === 'web-dashboard') {
                authenticated = true;
                ws.role = 'web-dashboard';
                session.setWebDashboardWs(ws);
                ws.send(JSON.stringify({
                  type: 'auth_response',
                  payload: { success: true, message: 'Web Dashboard authenticated' }
                }));
                logger.info('Web Dashboard client authenticated');
                return;
              }
        ...
        if (ws.role === 'web-dashboard') {
          ...
          if (mcWs && mcWs.readyState === 1) { // OPEN
            ...
            ws.send(JSON.stringify({
              type: 'error_response',
              payload: { message: '遊戲伺服器未連線', query_id: payload?.query_id }
            }));
          }
          return;
        }
    ...
    ws.on('close', () => {
      if (ws.role === 'web-dashboard') {
        if (session.getWebDashboardWs() === ws) {
          session.setWebDashboardWs(null);
